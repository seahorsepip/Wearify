                ...
                call.enqueue(new retrofit2.Callback<Paging<SavedAlbum>>() {
                    ...
                    public void onResponse(Call<Paging<SavedAlbum>> call, Response<Paging<SavedAlbum>> response) {
                        if (response.isSuccessful()) {
                            ...
                            Paging<SavedAlbum> albums = response.body();
                            for (SavedAlbum savedAlbum : albums.items) {
                                Item item = new Item();
                                item.setAlbum(savedAlbum.album);
                            ...
                            if (albums.total > albums.offset + limit) {
                                getAlbums(limit, albums.offset + limit);
                            }
                        }
                    }
